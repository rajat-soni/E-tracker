...
if (!fs.existsSync(dir1)){
    ...
    if(tact==="Email-Blast")
    {
        var dir3=`./files/${camp_id}/user/Email-Blast`;
    ...
    {
        var dir3=`./files/${camp_id}/user/Email-Reminder-Blast`;
...
// else{
//     var dir3=`./files/${camp_id}/user/${tact}`;
...
router.post("/comment",  upload.any('image'),(request, response) => {
    ...
    console.log("kdfksdfsjdk"+combinedFilenames);
...
console.log("hhhhh");
        ...
        if (camp_id != "") {
            ...
            var selectQuery = `select * from comment_tbl where camp_id = '${request.body.camp_id}'`; // verifying code id either exist or not //
            ...
            database.query(selectQuery, function (err, data) {
                if (err) {
                    ...
                    if (data.length > 0) {
                        ...
                        var updateSql = `UPDATE comment_tbl SET   user_ebfiles = '${combinedFilenames}', user_ebcomment = '${sendComment}' WHERE camp_id = '${request.body.camp_id}'`;
                    ...
                    console.log("Update query" +updateSql);
                        database.query(updateSql, function (err, data) {
                            ...
                            if (err) {
                                ...
                                response.json({
                                    success: true,
                                    message: 'Email blast updated  id exist'
                                })
                            }
    ...
    //console.log("admin_files11:" +admin_files11);
                ...
                if (err) {
                    ...
                    if (data.length > 0) {
                        ...
                        var updateSql = `UPDATE comment_tbl SET image='${newfile}' WHERE camp_id = '${camp_id}'`
                        // console.log(Object.keys(imagePath).map(function(k){return imagePath[k]}).join(","));
                        
                    //    if(tact==="Email-Reminder-Blast")
                    //    {
                    //     var updateSql = `UPDATE comment_tbl SET user_rb_file='${output}' WHERE camp_id = '${camp_id}'`;
                    ...
                    //    }
                        ...
                        var sqlQry = `INSERT INTO comment_tbl (camp_id,image) VALUES (${camp_id}, '${output}')`; // insert query //
                     //   console.log(Object.keys(imagePath).map(function(k){return imagePath[k]}).join(","));
    
                    //  if(tact="Email-Reminder-Blast")
                    //  {
                    //     var sqlQry = `INSERT INTO comment_tbl (camp_id,user_rb_file) VALUES (${camp_id}, '${output}')`; // insert query //
                    //  }
                    //  else{
                    //     var sqlQry = `INSERT INTO comment_tbl (camp_id,image) VALUES (${camp_id}, '${output}')`; // insert query //
                    //  }
