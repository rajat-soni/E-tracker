...
console.log(query);
       ...
       else if(tact=="webinar")
       {
		...
		INSERT INTO addtask 
		(cname,camp_name, camp_from,blast_count,asset_name,asset_link,tact,blast_type,blast_date,blast_time,rb_type,rb_assetname,rb_assetlink,rb_date,rb_time,comment,priority,allocated_to,status) 
		VALUES ("${cnamewebinar}", "${webcamp_name}","${webcamp_from}","${webinar_count}", "${webasset_name}", "${registration_link}", "${tact}", "${webblsttype}", "${registration_date}","${registration_time}", "${rb_type}", "${rb_assetname}","${rb_assetlink}","${rb_date}","${rb_time}","${webcomment}", "${webpriority}", "${weballocated_to}", "${webstatus}")
		`;
